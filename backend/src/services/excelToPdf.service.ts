...
/**
 * Excel → PDF by reading every worksheet's cell values with exceljs and
 * rendering each as an HTML table (one per page), then printing that
 * through Puppeteer. This reproduces cell text, basic per-cell layout, and
 * one page per sheet faithfully; it does not reproduce cell formatting
 * (fonts/colors/borders), formulas' underlying expressions (only their
 * computed values), charts, or images embedded in the sheet.
 */
export class ExcelToPdfService {
  ...
  async convert(body: ExcelToPdfBody): Promise<ProcessedFileDto> {
    ...
    const sheetsHtml = workbook.worksheets
      .map((sheet, i) => this.renderSheet(sheet, i > 0))
      .join("\n");
