 ...
 * subclass). The global error middleware knows how to render these into the
 * standard error envelope; anything else it catches is treated as an
 * unexpected 500 and logged for investigation.
 */
export class ApiError extends Error {
  public readonly status: number;
  public readonly errors: ApiErrorDetail[];
  /** True for errors we deliberately threw (bad input, not found, ...) as
   *  opposed to bugs — used to decide how much detail is safe to expose. */
